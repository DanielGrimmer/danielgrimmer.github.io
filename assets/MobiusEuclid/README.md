...
| `GameDemos.decoded.txt` (Sep 2025) | **The object catalogue, recovered.** The source of `GameDemos.nb`, a later and much richer Mathematica notebook than `MobiusEuclidGame.nb`: the full eight-type object format (`F` floor, `D` cylinder, `L` wall, `H` wedge, `V` slab, `B` block, `E` Euclid, `M` Möbius), each with a dual pair of renderers, plus the three background scenes and the scripted trajectories the GIFs were rendered from. Extracted from the notebook's box structures — the `.nb` itself was 25 MB, almost entirely embedded output images, and is deliberately **not** kept here; the author holds the original. |
| `MobiusEuclidGame.nb` (Sep 2025) | The recovered Mathematica notebook behind the pitch imagery — the closest thing the project has to a design document. It defines the world constants, the Radon/InverseRadon floor pipeline, a declarative object format (`D` cylinders, `L` walls, `E`/`M` players) rendered into **both** views by a pair of scene compilers, and the three levels as pure data. Kept here for reference; its functionality now lives in `floors.html`, `catalogue.html`, and `levels.html`. Superseded by `GameDemos.nb` above. |
...
| `catalogue.html` (Aug 2026) | The object catalogue: a bare playing field with one of every shape, walkable at one keyboard, each object selectable and captioned in both frames. The first six (cylinder, block, wedge, point marker, strip block, strip wedge) were rebuilt from memory before `GameDemos.nb` turned up; the notebook's three missing types were then added from it — **Bowtie (H)**, every line through a point for a window of directions; **Slab (V)**, the lines between two parallel lines, which is a segment of one *fibre* in Möbius's world; and **Clipped band (B)**, every line separating two points, cut off outside an angular window. The mechanics sandbox for demos and the pitch. |
...
| `levels.html` (Aug 2026) | The notebook, ported: the notebook's units with a **disk arena of radius 3π/2**, matching the strip's r range exactly, so the lines Möbius can stand on are precisely those meeting Euclid's field (a square arena let him walk clean off it); strip θ ∈ [π/2 ± 3π/2] shown as three tiles, floor text flipped per tile parity, dual line/section bundles on both floors, Möbius's 50 %-opacity ghost copies one period over, and both players walkable at one keyboard with collision in each frame. It now runs `GameDemos.nb`'s **full eight-type record format** — `F` floor, `D` cylinder, `L` wall, `H` bowtie, `V` slab, `B` clipped band, `E`/`M` spawns — through a scene compiler that builds both worlds and both collision sets in one pass, exactly as the notebook's `drawEuc`/`drawMob` do. Levels 1–3 are `background1..3` verbatim; levels 4–8 are one per object type with exactly one of that object, for looking at each dual pair on its own. **F cycles three view modes** — overhead, third person (a chase camera behind each player, following his yaw), and first person (down at their eyes). Both look modes take pointer-lock mouse control: one mouse and two players, so it claims whichever panel you click on (Esc releases, then click the other; Q swaps in play), and that player walks relative to his gaze while the other turns to face the way he walks. |
